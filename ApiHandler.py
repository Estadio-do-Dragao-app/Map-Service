...
@app.get("/tiles", response_model=List[TileResponse])
def get_tiles(db: Session = Depends(get_db)):
    """Get all tiles."""
    return db.query(Tile).all()

@app.get("/tiles/{tile_id}", response_model=TileResponse)
...
# @app.post("/tiles", response_model=TileResponse, status_code=201)
...
@app.put("/tiles/{tile_id}", response_model=TileResponse)
